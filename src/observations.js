...
function observationSummary(data) {
  for (let i = 0; i < data.results.length; i++) {
    ...
    console.log(`ID-${id} "${speciesGuess}" (Date: ${date})`);
  }
}
...
function observationsByGeoPrivacy(data, geoPrivacy) {
  const allowedValues = ['open', 'obscured', 'private', null];
  if (!allowedValues.includes(geoPrivacy && geoPrivacy.toLowerCase())) {
    throw new Error(`Invalid geoPrivacy value: ${geoPrivacy}`);
  }
  const matchingObservations = [];
  for (const observation of data.results) {
    const observationGeoPrivacy = observation.geoprivacy;
    if (
      observationGeoPrivacy === geoPrivacy ||
      observationGeoPrivacy === (geoPrivacy && geoPrivacy.toLowerCase())
    ) {
...
function transformObservation(original) {
  const transformed = {
    ...
    coords: original.location.split(',').reverse().map(Number),
    photos: original.photos.map((p) => p.url),
...
function getObservationsById(data, ...ids) {
  if (ids.length === 1) {
    const id = ids[0];
    const result = data.results.find((obs) => obs.id === id);
    return result || null;
  }
  const results = ids
    .map((id) => data.results.find((obs) => obs.id === id))
...
function getObservationsByPositionalAccuracy(data, options = {}) {
  ...
  const { eq, min, max, gt, lt } = options;
  return data.results.filter((observation) => {
    const posAcc = parseFloat(observation.positional_accuracy);
    if (!isNaN(posAcc)) {
...
function getUserStats(data) {
  ...
  data.results.forEach((result) => {
    ...
    journals_count += user.journal_posts_count;
...
function extractTimeZones(data) {
  let timeZoneValues = [];
  data.results.forEach((result) => {
    if (!timeZoneValues.includes(result.created_time_zone)) {
      timeZoneValues.push(result.created_time_zone);
    }
    if (!timeZoneValues.includes(result.observed_time_zone)) {
      timeZoneValues.push(result.observed_time_zone);
    }
  });
...
function extractTimeZones2(data) {
  ...
  for (let result of data.results) {
    set.add(result.created_time_zone);
    set.add(result.observed_time_zone);
  }
